...
for(did in dataIDs){
  ...
  ercofile = paste(ercodir,'ij2hr-',did,'-cw-vv.dat',sep="");
  ercodata[[did]] <- read.table(file=ercofile,sep = "",head = FALSE,skip = 4)

  ploteDataTemp <- data.frame(y=ercodata[[did]][,1],x=ercodata[[did]][,2]*(uBulk^2),colname = rep(did,length(ercodata[[did]][,1])),xoff = rep(xoffset,length(ercodata[[did]][,1])))
...
for(did in dataIDs){
  ...
  plotDataTemp <- data.frame(y=simY[[did]]/pipeD,x=simPhi[[did]],colname = rep(did,length(simY[[did]])),xoff = rep(xoffset,length(simY[[did]])))
...
termplot <- ggplot()+
  ...
  facet_grid(~ colname)
   ...
   ggsave(filename="vv_ijet.pdf", plot=termplot, width=9, height=9)
